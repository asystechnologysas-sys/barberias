...
import { Calendar, Users, LogOut, Lock, Unlock, X, ChevronLeft, ChevronRight, AlertTriangle } from 'lucide-react';
import { api } from '../api';
...
export default function BarberDashboard() {
  ...
  // Fecha seleccionada en el Almanaque
  ...
  // Normalizador de hora en formato colombiano
  ...
  // Días del mes para el Almanaque Máster
  ...
  const monthDays = Array.from({ length: daysInMonth }, (_, i) => {
    ...
    const occupiedCount = dayApts.length + dayBlocks.length;
    return {
      dayNum: i + 1,
      dateStr,
      occupiedCount
  ...
  return (
    <div className="admin-page-wrap">
        ...
        <div>
          {activeTab === 'schedule' ? (
            <div className="client-calendar-card">
              ...
              {/* Días del Mes */}
              <div className="cal-days-grid" style={{ marginBottom: 24 }}>
                {monthDays.map(d => (
                  <div
                    key={d.dateStr}
                      ...
                      <div key={h} className="master-slot-row slot-booked">
                        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                          ...
                          <div>
                            ...
                            <b style={{ display: 'block', fontSize: 15, marginTop: 4 }}>{apt.clientName}</b>
            ...
            /* Clientes VIP */
            <div className="client-calendar-card">
              <span className="cal-eyebrow">CLIENTES VIP</span>
              <h2 style={{ fontFamily: 'Sora', fontSize: 22, marginBottom: 20 }}>Turnos Fijos Semanales</h2>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {vips.map(v => (
                  <div key={v.id} className="vip-card-clean">
                    <div>
                      <b>{v.client?.name || 'Cliente VIP'}</b>
                      <span style={{ fontSize: 13, color: '#64748b', display: 'block' }}>
                        Día {v.weekday === 6 ? 'Sábado' : 'Viernes'} a las {v.time} · Tel: {v.client?.phone}
                      </span>
                    </div>
      ...
      {/* MODAL FLOTANTE DE CONFIRMACIÓN ASYS */}
        ...
        <div className="modal-hours-overlay">
          <div className="modal-hours-box" style={{ maxWidth: 420, textAlign: 'center' }}>
            ...
            <p style={{ color: '#64748b', fontSize: 13, lineHeight: 1.5, marginBottom: 20 }}>
              La cita se marcará como cancelada y la hora volverá a quedar disponible inmediatamente para otros clientes.
            </p>
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
              <button onClick={() => setConfirmModal(null)} className="btn-dark" style={{ width: '50%' }}>
                No, mantener
              </button>
              <button onClick={handleCancelAppointment} className="btn-action-sm btn-cancel" style={{ width: '50%', padding: 12 }}>
                Sí, cancelar
              </button>
            </div>
          </div>
